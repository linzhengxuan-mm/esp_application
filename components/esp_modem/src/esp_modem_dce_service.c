...
static esp_err_t esp_modem_dce_handle_user_cmd(modem_dce_t *dce, const char *line)
{
	esp_err_t err = ESP_FAIL;
	if (strstr(line, MODEM_RESULT_CODE_SUCCESS)) {
		err = esp_modem_process_command_done(dce, MODEM_STATE_SUCCESS);
		...
		esp_modem_dce_t *esp_dce = __containerof(dce, esp_modem_dce_t, parent);
		if (esp_dce->user_cmd_cb)
		{
			err = esp_dce->user_cmd_cb(line);
		}
	}
	return err;
}
...
esp_err_t esp_modem_dce_send_user_cmd(modem_dce_t *dce, const char *command,uint32_t timeout,modem_dce_user_cmd_cb_t cmd_cb)
{
    modem_dte_t *dte = dce->dte;
    esp_modem_dce_t *esp_dce = __containerof(dce, esp_modem_dce_t, parent);
    dce->handle_line = esp_modem_dce_handle_user_cmd;
	esp_dce->user_cmd_cb = cmd_cb;
    DCE_CHECK(dte->send_cmd(dte, command, timeout) == ESP_OK, "send user command failed", err);
    DCE_CHECK(dce->state == MODEM_STATE_SUCCESS, "send user command state error", err);
    ESP_LOGD(DCE_TAG, "dce send user common cmd ok");
    return ESP_OK;
err:
    return ESP_FAIL;
}
