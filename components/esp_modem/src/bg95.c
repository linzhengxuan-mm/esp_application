...
static const char *DCE_TAG = "bg95";
 ...
 */
static esp_err_t bg95_handle_power_down(modem_dce_t *dce, const char *line)
{
 ...
 *      - ESP_FAIL on error
 */
static esp_err_t bg95_set_working_mode(modem_dce_t *dce, modem_mode_t mode)
{
 ...
 * @brief Power down
 *
 * @param bg95_dce bg95 object
 * @return esp_err_t
 *      - ESP_OK on success
 *      - ESP_FAIL on error
 */
static esp_err_t bg95_power_down(modem_dce_t *dce)
{
    modem_dte_t *dte = dce->dte;
    dce->handle_line = bg95_handle_power_down;
...
/**
 * @brief Deinitialize BG95 object
 *
 * @param dce Modem DCE object
 * @return esp_err_t
 *      - ESP_OK on success
 *      - ESP_FAIL on fail
 */
static esp_err_t bg95_deinit(modem_dce_t *dce)
{
...
modem_dce_t *bg95_init(modem_dte_t *dte)
{
    ...
    DCE_CHECK(esp_modem_dce, "calloc bg95_dce failed", err);
    /* Bind DTE with DCE */
    ...
    esp_modem_dce->parent.send_user_cmd= esp_modem_dce_send_user_cmd;
    esp_modem_dce->parent.set_working_mode = bg95_set_working_mode;
    esp_modem_dce->parent.power_down = bg95_power_down;
    esp_modem_dce->parent.deinit = bg95_deinit;
